...
export const suggestHabits = async (req, res) => {
  try {
    ...
    const userMsg =
      ...
      `Current struggles: ${struggles || "not provided"}`;
...
export const recoveryPlan = async (req, res) => {
  try {
    ...
    const userMsg =
      ...
      `Create a recovery plan if the user has fallen behind.`;
  ...
  try {
    ...
    const context = habits
      .map((h) => {
        ...
        return `${h.name} (${h.category}): ${hLogs.length}/30 in last 30 days, by weekday [${byDow.join(
          ","
        )}]`;
      })
      .join("\n");
